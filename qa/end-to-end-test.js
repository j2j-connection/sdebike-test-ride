#!/usr/bin/env node

/**
 * SDEBIKE Test Ride App - End-to-End Flow Test
 * 
 * This script simulates a complete user journey through the test ride signup process
 * to verify all functionality works correctly from start to finish.
 */

const http = require('http');
const https = require('https');

// Test configuration
const TEST_CONFIG = {
  baseUrl: 'http://localhost:3000',
  timeout: 20000,
  testData: {
    customer: {
      name: 'QA Test Customer',
      phone: '555-987-6543',
      email: 'qa-test@example.com'
    },
    bike: 'Test Electric Bike',
    signature: 'QA Test Signature'
  }
};

// Test results tracking
let testResults = {
  total: 0,
  passed: 0,
  failed: 0,
  errors: [],
  testRideId: null
};

/**
 * Utility function to make HTTP requests
 */
function makeRequest(url, options = {}) {
  return new Promise((resolve, reject) => {
    const urlObj = new URL(url);
    const isHttps = urlObj.protocol === 'https:';
    const client = isHttps ? https : http;
    
    const requestOptions = {
      hostname: urlObj.hostname,
      port: urlObj.port || (isHttps ? 443 : 80),
      method: options.method || 'GET',
      headers: options.headers || {},
      timeout: TEST_CONFIG.timeout
    };

    if (urlObj.pathname) {
      requestOptions.path = urlObj.pathname + urlObj.search;
    }

    const req = client.request(requestOptions, (res) => {
      let data = '';
      res.on('data', (chunk) => data += chunk);
      res.on('end', () => {
        resolve({
          statusCode: res.statusCode,
          headers: res.headers,
          data: data
        });
      });
    });

    req.on('error', reject);
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Request timeout'));
    });

    if (options.body) {
      req.write(options.body);
    }
    req.end();
  });
}

/**
 * Test runner function
 */
async function runTest(testName, testFunction) {
  testResults.total++;
  console.log(`\n🧪 Running: ${testName}`);
  
  try {
    await testFunction();
    console.log(`✅ PASSED: ${testName}`);
    testResults.passed++;
  } catch (error) {
    console.log(`❌ FAILED: ${testName}`);
    console.log(`   Error: ${error.message}`);
    testResults.failed++;
    testResults.errors.push({ test: testName, error: error.message });
  }
}

/**
 * Test: Application Initial Load
 */
async function testInitialLoad() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Verify the signup form is loaded
  const formChecks = [
    'San Diego Electric Bike',
    'Test Ride Sign-Up',
    'Contact Info',
    'Full Name',
    'Phone Number',
    'Email Address'
  ];
  
  for (const check of formChecks) {
    if (!response.data.includes(check)) {
      throw new Error(`Missing form element: ${check}`);
    }
  }
  
  console.log('   ℹ️  Signup form loaded successfully');
}

/**
 * Test: Payment Intent Creation
 */
async function testPaymentIntentCreation() {
  const testData = {
    amount: 100,
    customerEmail: TEST_CONFIG.testData.customer.email,
    testRideId: `qa-test-${Date.now()}`
  };

  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/api/create-payment-intent`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify(testData)
  });

  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }

  const responseData = JSON.parse(response.data);
  if (!responseData.clientSecret || !responseData.paymentIntentId) {
    throw new Error('Response missing required payment intent data');
  }

  testResults.testRideId = testData.testRideId;
  console.log(`   ℹ️  Payment Intent created: ${responseData.paymentIntentId}`);
  console.log(`   ℹ️  Test Ride ID: ${testResults.testRideId}`);
}

/**
 * Test: Success Page Accessibility
 */
async function testSuccessPage() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/test-ride-success`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  const successChecks = [
    'Test Ride Confirmed',
    'Your payment has been authorized',
    'Pick up your bike',
    '30-minute test ride',
    'Stay in the area'
  ];
  
  for (const check of successChecks) {
    if (!response.data.includes(check)) {
      throw new Error(`Missing success content: ${check}`);
    }
  }
  
  console.log('   ℹ️  Success page displays all required information');
}

/**
 * Test: Admin Dashboard Structure
 */
async function testAdminDashboard() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/admin`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Check for admin dashboard structure
  const structureChecks = [
    'SDEBIKE Admin',
    'Search name, phone, bike',
    'Refresh'
  ];
  
  for (const check of structureChecks) {
    if (!response.data.includes(check)) {
      throw new Error(`Missing admin dashboard structure: ${check}`);
    }
  }

  // Check if it shows loading or no data (both are valid states)
  if (response.data.includes('Loading')) {
    console.log('   ℹ️  Admin dashboard is loading data');
  } else if (response.data.includes('No active test rides')) {
    console.log('   ℹ️  Admin dashboard shows no active test rides');
  } else {
    console.log('   ℹ️  Admin dashboard loaded successfully');
  }
}

/**
 * Test: Static Assets and Resources
 */
async function testStaticAssets() {
  const assets = [
    '/favicon.ico',
    '/logo.png'
  ];

  for (const asset of assets) {
    try {
      const response = await makeRequest(`${TEST_CONFIG.baseUrl}${asset}`);
      if (response.statusCode !== 200) {
        throw new Error(`Asset ${asset} returned status ${response.statusCode}`);
      }
    } catch (error) {
      console.log(`   ⚠️  Asset ${asset}: ${error.message}`);
    }
  }
  
  console.log('   ℹ️  Static assets are accessible');
}

/**
 * Test: Form Validation Structure
 */
async function testFormValidation() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Check for form validation elements
  const validationChecks = [
    'required',
    'type="text"',
    'type="tel"',
    'type="email"',
    'placeholder'
  ];
  
  let validationScore = 0;
  for (const check of validationChecks) {
    if (response.data.includes(check)) {
      validationScore++;
    }
  }
  
  if (validationScore < 3) {
    throw new Error(`Insufficient form validation: ${validationScore}/5`);
  }
  
  console.log(`   ℹ️  Form validation score: ${validationScore}/5`);
}

/**
 * Test: Responsive Design Elements
 */
async function testResponsiveDesign() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Check for responsive design indicators
  const responsiveChecks = [
    'viewport',
    'max-w-md',
    'grid-cols-1',
    'md:grid-cols-2'
  ];
  
  let responsiveScore = 0;
  for (const check of responsiveChecks) {
    if (response.data.includes(check)) {
      responsiveScore++;
    }
  }
  
  if (responsiveScore < 2) {
    throw new Error(`Insufficient responsive design elements: ${responsiveScore}/4`);
  }
  
  console.log(`   ℹ️  Responsive design score: ${responsiveScore}/4`);
}

/**
 * Test: Performance and Loading
 */
async function testPerformance() {
  const startTime = Date.now();
  
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  const endTime = Date.now();
  const responseTime = endTime - startTime;
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  if (responseTime > 3000) {
    throw new Error(`Response time too slow: ${responseTime}ms`);
  }
  
  console.log(`   ℹ️  Response time: ${responseTime}ms`);
  
  // Check for performance optimizations
  const performanceChecks = [
    'async',
    'defer',
    'preload'
  ];
  
  let performanceScore = 0;
  for (const check of performanceChecks) {
    if (response.data.includes(check)) {
      performanceScore++;
    }
  }
  
  console.log(`   ℹ️  Performance optimizations: ${performanceScore}/3`);
}

/**
 * Test: Security and Headers
 */
async function testSecurity() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Check for security-related content
  const securityChecks = [
    'https:',
    'secure',
    'csrf',
    'xss'
  ];
  
  let securityScore = 0;
  for (const check of securityChecks) {
    if (response.data.includes(check)) {
      securityScore++;
    }
  }
  
  console.log(`   ℹ️  Security indicators: ${securityScore}/4`);
}

/**
 * Test: Accessibility Features
 */
async function testAccessibility() {
  const response = await makeRequest(`${TEST_CONFIG.baseUrl}/`);
  
  if (response.statusCode !== 200) {
    throw new Error(`Expected status 200, got ${response.statusCode}`);
  }
  
  // Check for basic accessibility features
  const accessibilityChecks = [
    'alt=',
    'label',
    'aria-',
    'role='
  ];
  
  let accessibilityScore = 0;
  for (const check of accessibilityChecks) {
    if (response.data.includes(check)) {
      accessibilityScore++;
    }
  }
  
  if (accessibilityScore < 2) {
    console.log(`   ⚠️  Low accessibility score: ${accessibilityScore}/4`);
  } else {
    console.log(`   ℹ️  Accessibility score: ${accessibilityScore}/4`);
  }
}

/**
 * Main test execution
 */
async function runAllTests() {
  console.log('🚀 Starting SDEBIKE Test Ride App - End-to-End Flow Test');
  console.log(`📍 Testing application at: ${TEST_CONFIG.baseUrl}`);
  console.log(`⏱️  Timeout per test: ${TEST_CONFIG.timeout}ms`);
  console.log(`👤 Test customer: ${TEST_CONFIG.testData.customer.name}`);
  console.log(`📧 Test email: ${TEST_CONFIG.testData.customer.email}`);
  
  const startTime = Date.now();
  
  // Run all tests in logical order
  await runTest('Application Initial Load', testInitialLoad);
  await runTest('Payment Intent Creation', testPaymentIntentCreation);
  await runTest('Success Page Accessibility', testSuccessPage);
  await runTest('Admin Dashboard Structure', testAdminDashboard);
  await runTest('Static Assets and Resources', testStaticAssets);
  await runTest('Form Validation Structure', testFormValidation);
  await runTest('Responsive Design Elements', testResponsiveDesign);
  await runTest('Performance and Loading', testPerformance);
  await runTest('Security and Headers', testSecurity);
  await runTest('Accessibility Features', testAccessibility);
  
  const endTime = Date.now();
  const duration = (endTime - startTime) / 1000;
  
  // Print results
  console.log('\n' + '='.repeat(70));
  console.log('📊 END-TO-END FLOW TEST RESULTS SUMMARY');
  console.log('='.repeat(70));
  console.log(`Total Tests: ${testResults.total}`);
  console.log(`✅ Passed: ${testResults.passed}`);
  console.log(`❌ Failed: ${testResults.failed}`);
  console.log(`⏱️  Duration: ${duration.toFixed(2)}s`);
  console.log(`📈 Success Rate: ${((testResults.passed / testResults.total) * 100).toFixed(1)}%`);
  
  if (testResults.testRideId) {
    console.log(`🆔 Test Ride ID: ${testResults.testRideId}`);
  }
  
  if (testResults.failed > 0) {
    console.log('\n❌ FAILED TESTS:');
    testResults.errors.forEach(error => {
      console.log(`   • ${error.test}: ${error.error}`);
    });
  }
  
  if (testResults.passed === testResults.total) {
    console.log('\n🎉 All end-to-end tests passed! Application is fully functional.');
    console.log('✅ Ready for production deployment');
    console.log('✅ All user flows working correctly');
    console.log('✅ Payment integration functional');
    console.log('✅ Admin dashboard accessible');
  } else if (testResults.passed >= testResults.total * 0.9) {
    console.log('\n⚠️  Most tests passed. Application is highly functional.');
    console.log('📋 Review failed tests before production deployment');
  } else {
    console.log('\n🚨 Many tests failed. Application needs significant work.');
    console.log('🔧 Fix critical issues before proceeding');
  }
  
  console.log('\n📝 Next Steps:');
  if (testResults.failed > 0) {
    console.log('   1. Fix failed tests');
    console.log('   2. Re-run end-to-end tests');
    console.log('   3. Address any warnings or low scores');
  } else {
    console.log('   1. ✅ All automated tests passed');
    console.log('   2. 🔍 Run manual user flow testing');
    console.log('   3. 💳 Test payment flow with real Stripe test cards');
    console.log('   4. 📱 Test on mobile devices');
    console.log('   5. 🚀 Prepare for production deployment');
  }
  
  console.log('\n🔍 Manual Testing Checklist:');
  console.log('   • Complete test ride signup flow with real data');
  console.log('   • Test file uploads (ID photos, signatures)');
  console.log('   • Verify payment flow with Stripe test cards');
  console.log('   • Test admin dashboard with real test ride data');
  console.log('   • Test responsive design on various screen sizes');
  console.log('   • Test accessibility with screen readers');
  
  console.log('\n🎯 Production Readiness:');
  if (testResults.passed === testResults.total) {
    console.log('   ✅ All core functionality working');
    console.log('   ✅ Payment integration verified');
    console.log('   ✅ User flows tested');
    console.log('   ✅ Admin features accessible');
    console.log('   🚀 READY FOR PRODUCTION');
  } else {
    console.log('   ⚠️  Some issues need resolution');
    console.log('   🔧 Fix remaining problems first');
  }
}

// Handle command line arguments
if (process.argv.includes('--help') || process.argv.includes('-h')) {
  console.log(`
SDEBIKE Test Ride App - End-to-End Flow Test

Usage: node qa/end-to-end-test.js [options]

Options:
  --help, -h     Show this help message
  --url <url>    Set custom base URL (default: http://localhost:3000)
  --timeout <ms> Set request timeout in milliseconds (default: 20000)

Examples:
  node qa/end-to-end-test.js
  node qa/end-to-end-test.js --url http://localhost:3001
  node qa/end-to-end-test.js --timeout 30000
  `);
  process.exit(0);
}

// Parse command line arguments
const urlIndex = process.argv.indexOf('--url');
if (urlIndex !== -1 && process.argv[urlIndex + 1]) {
  TEST_CONFIG.baseUrl = process.argv[urlIndex + 1];
}

const timeoutIndex = process.argv.indexOf('--timeout');
if (timeoutIndex !== -1 && process.argv[timeoutIndex + 1]) {
  TEST_CONFIG.timeout = parseInt(process.argv[timeoutIndex + 1]);
}

// Run tests
runAllTests().catch(error => {
  console.error('\n💥 End-to-end test runner failed:', error.message);
  process.exit(1);
});
